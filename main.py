...
def saving(xi, answerLists_y, labelList):
    n = len(str(len(xi)))
    output_text = "\nUsing Euler methods:\n\n    " + (n-1)*" " + "i    xi          "
...
def plotting(x_collection, y_collection, label_collection, func, h):
    """
...
def main() -> None:
    ...
    doExactSolve    = True if input_lines[3] == 'y' else False
    ...
    for method_no in use_methods:
        ...
        answerLists_y.append(switch(method_no, f, x0, y0, xn, h))
        answerLabels.append(METHODS[method_no])
    ...
    with open('Output/output.txt', 'w') as output_:
        output_.truncate(0)
    saving(answerList_x, answerLists_y, answerLabels)

    ########################################    

    plotting(answerList_x, answerLists_y, answerLabels, f_input, h)
